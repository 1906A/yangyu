    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="upOrDown(props.item)">下架</v-btn>
          <v-btn icon v-else @click="upOrDown(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      upOrDown(good){
        ...
        this.$message.confirm(msg, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {

          this.$http.get("/item/spu/editSpuStatus?id="+good.id+"&saleable="+(!good.saleable))
            .then(({data}) => {
      ...
      deleteGoods(good){
        ...
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          ...
          this.$http.get("/item/spu/deleteSpuStatus?id="+good.id+"&valid=false")
            .then(({data}) => {
      ...
      async editGoods(oldGoods) {
        // 发起请求，查询商品详情和skus
        let spu = await this.$http.loadData("/item/spu/baseInfo/" + oldGoods.id);
        ...
        oldGoods.cid1 = spu.cid1;
        oldGoods.cid2 = spu.cid2;
        oldGoods.cid3 = spu.cid3;
        oldGoods.brandId = spu.brandId;
